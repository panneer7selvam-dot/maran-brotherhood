...
const INITIAL_KIDS = [
  { 
    ...
    credits: 0, // ₹ Rupees (Spendable)
    xp: 0,      // ✨ Wisdom (Growth)
    level: 1, str: 10, eq: 10,
    battery: 80, streak: 0, 
...
export default function MaranEcosystem() {
  ...
  // --- PERSISTENCE ---
  useEffect(() => {
    const savedKids = localStorage.getItem('maran_kids_v9');
    const savedQuests = localStorage.getItem('maran_quests_v9');
    const savedLogs = localStorage.getItem('maran_logs_v9');
  ...
  useEffect(() => {
    localStorage.setItem('maran_kids_v9', JSON.stringify(kids));
    localStorage.setItem('maran_quests_v9', JSON.stringify(quests));
    localStorage.setItem('maran_logs_v9', JSON.stringify(soulLogs));
  ...
  const handleMoodSelect = (mood: any) => {
    ...
    updateStat(activeKid.id, 'eq', 5); // EQ Reward
        ...
        {activeTab === 'DOJO' && (
           <div className="animate-in fade-in slide-in-from-bottom-2">
              <div className="bg-red-900/20 border border-red-500/30 p-6 rounded-2xl text-center mb-6">
                 ...
                 <p className="text-xs text-red-300/70">Train your body. Earn Strength.</p>
              </div>
