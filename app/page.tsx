...
import { 
  ...
  Thermometer, Activity, Brain, BicepsFlexed
} from 'lucide-react';
...
const INITIAL_KIDS = [
  { 
    ...
    credits: 150, // Wealth
    xp: 850, level: 5, // General Rank
    str: 20, // Physical Strength
    eq: 15,  // Emotional Intelligence
    battery: 80, 
    streak: 4, 
...
export default function MaranHeroApp() {
  ...
  // --- PERSISTENCE ---
  useEffect(() => {
    const savedKids = localStorage.getItem('maran_kids_v6');
    const savedQuests = localStorage.getItem('maran_quests_v6');
    const savedLogs = localStorage.getItem('maran_logs_v6');
  ...
  useEffect(() => {
    localStorage.setItem('maran_kids_v6', JSON.stringify(kids));
    localStorage.setItem('maran_quests_v6', JSON.stringify(quests));
    localStorage.setItem('maran_logs_v6', JSON.stringify(soulLogs));
  ...
  const handleMoodSelect = (mood: any) => {
    ...
    // Reward for identifying emotion (EQ Gain)
    updateStat(activeKid.id, 'eq', 2);
    setTimeout(() => setCurrentMoodAdvice(null), 8000); // Hide advice after 8s
  };
  ...
  const addSoulLog = () => {
    ...
    setNewLog('');
    updateStat(activeKid.id, 'eq', 5); // Writing feelings = EQ gain
  };
        ...
        {activeTab === 'DOJO' && (
           <div className="animate-in fade-in slide-in-from-bottom-2">
              <div className="bg-red-900/20 border border-red-500/30 p-6 rounded-2xl text-center mb-6">
                 ...
                 <h2 className="text-xl font-black text-red-100 uppercase italic">The Training Dojo</h2>
                 <p className="text-xs text-red-300/70">"Pain is weakness leaving the body."</p>
              </div>

              <h3 className="text-xs font-bold text-slate-500 uppercase mb-3 ml-1">Training Menu</h3>
              <div className="grid grid-cols-2 gap-3">
                 {EXERCISES.map(ex => (
                    <div key={ex.id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 hover:border-red-500/50 transition relative overflow-hidden group">
        ...
        {activeTab === 'MIND' && (
           <div className="animate-in fade-in slide-in-from-bottom-2">
              <div className="bg-indigo-900/20 border border-indigo-500/30 p-6 rounded-2xl text-center mb-6">
                 ...
                 <h2 className="text-xl font-black text-indigo-100 uppercase italic">The Emotion Lab</h2>
                 <p className="text-xs text-indigo-300/70">Scan your mood to receive protocols.</p>
              </div>
              ...
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 mb-6">
                 ...
                 <div className="flex justify-between gap-2">
                    {MOODS.map(m => (
                       <button key={m.label} onClick={() => handleMoodSelect(m)} className="flex-1 flex flex-col items-center bg-slate-800 p-3 rounded-xl hover:bg-slate-700 border border-slate-700">
                          <span className="text-2xl mb-1 grayscale hover:grayscale-0 transition">{m.icon}</span>
                 ...
                 {currentMoodAdvice && (
                    <div className="mt-4 bg-blue-600/20 border border-blue-500/50 p-3 rounded-xl text-center animate-in zoom-in duration-300">
                       ...
                       <p className="text-[10px] text-blue-400 mt-1">+2 EQ Points</p>
                    </div>
                 )}
              </div>
              ...
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800">
                 ...
                 <div className="mt-4 space-y-2">
                    {soulLogs.filter(l => l.kidId === activeKid.id).slice(0,3).map(l => (
                       <div key={l.id} className="text-xs bg-slate-800 p-2 rounded border-l-2 border-indigo-500 text-slate-300 italic">"{l.text}"</div>
                    ))}
                 </div>
              </div>
           </div>
